...
ros::Publisher pub_mapping_pose;
...
void registerPub(ros::NodeHandle &n)
{
    ...
    //TODO mapping and fusion
    pub_camera_pose = n.advertise<nav_msgs::Odometry>("/vins_estimator/fusion_pose", 1000);
    pub_camera_info = n.advertise<sensor_msgs::CameraInfo>("/vins_estimator/camera_info", 1000);
    pub_mapping_pose = n.advertise<geometry_msgs::PoseStamped>("/vins_estimator/mapping_pose", 1000);
...
void
pubOdometry(const Estimator &estimator, const std_msgs::Header &pose_header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        nav_msgs::Odometry odometry;
        odometry.header = pose_header;
        odometry.header.frame_id = "world";
        ...
        foutC.precision(0);
        foutC << pose_header.stamp.toSec() * 1e9 << ",";
        foutC.precision(5);
...
void
pubCameraPose(const Estimator &estimator, const std_msgs::Header &pose_header, const std_msgs::Header &mapping_header)
{
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        //TODO:Pub pub mapping pose
        geometry_msgs::PoseStamped mapping_pose;

        mapping_pose.header = mapping_header;
        string frame_id = to_string(mapping_header.stamp.sec)+to_string(mapping_header.stamp.nsec);

        mapping_pose.header.frame_id = frame_id;

//        mapping_pose.header.frame_id = ;

        mapping_pose.pose.position.x = P.x();
        mapping_pose.pose.position.y = P.y();
        mapping_pose.pose.position.z = P.z();

        mapping_pose.pose.orientation.x = R.x();
        mapping_pose.pose.orientation.y = R.y();
        mapping_pose.pose.orientation.z = R.z();
        mapping_pose.pose.orientation.w = R.w();
        pub_mapping_pose.publish(mapping_pose);
