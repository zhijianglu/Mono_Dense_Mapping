...
void quadmap::Publisher::publishDepthmap(ros::Time msg_time)
{
  ...
  cv_image.image = depthmap_mat;
    ...
    if(RECORD_DEPTH)
    {
//    imshow("depthmap_mat", cv_image.image);
//    cv::waitKey(1);

//show
        cv::Mat depth_img(depthmap_mat.size(), CV_16UC1);
        depthmap_mat.convertTo(depth_img, CV_16UC1);
//    cv::imshow("depth_raw",depth_viz);
//    cv::waitKey(1);
//write
        cv::imwrite(OUTPUT_PATH + "/depth_img/" + to_string(msg_time.toSec()) + ".jpg", depthmap_->getDepthmap());
    }
  ...
  // min = 0; max = 65;
  // min = 10; max = 30;
  depthmap_mat.convertTo(adjMap,CV_8UC1, 255/(max-min), -min);//转换成颜色编码的深度图
  cv::Mat falseColorsMap;
  cv::applyColorMap(adjMap, falseColorsMap, cv::COLORMAP_RAINBOW);
  cv::Mat mask;
